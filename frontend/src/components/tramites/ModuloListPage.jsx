...
export default function ModuloListPage({ titulo, keywords, nuevoPath, detallePath, accentColor = 'blue', IconoVacio }) {
    ...
    tramitesService.getMisSolicitudes()
      .then(({ data }) => {
        const lista = (data.solicitudes ?? data).filter(s =>
          keywords.some(k => s.tipoTramite?.nombre?.toLowerCase().includes(k.toLowerCase()))
        )
        setTodas(lista)
      })
